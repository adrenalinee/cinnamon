...
import static org.cinnamon.core.config.baseData.BaseDataBuilder.group;
...
import org.cinnamon.core.config.baseData.GroupWrapper;
...
public class CoreGroupSystemConfigurer implements SystemConfigurer {
	...
	public void configure(BaseDataBuilder baseData) {
		...
		baseData.addGroups(
			group("사용상태", Groups.useStatus).addChildGroup(
				group("사용중", UseStatus.enable),
				group("미사용중", UseStatus.disable)
			)
		).addGroups(
			group("메뉴 위치", Groups.menuPositions).addChildGroup(
	...
	private void addNationGroups(BaseDataBuilder baseData) {
		GroupWrapper nations = group("국가", Groups.nations);
